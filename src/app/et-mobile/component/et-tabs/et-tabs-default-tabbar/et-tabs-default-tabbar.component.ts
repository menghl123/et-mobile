import {AfterViewInit, Component, ContentChildren, ElementRef, HostBinding, Input, QueryList, Renderer2, ViewChild} from '@angular/core';

export type TabBarPositionType = 'top' | 'bottom' | 'left' | 'right';

@Component({
  selector: 'et-tabs-default-tabbar',
  templateUrl: './et-tabs-default-tabbar.component.html',
  styleUrls: ['./et-tabs-default-tabbar.component.scss']
})
export class EtTabsDefaultTabbarComponent implements AfterViewInit {
  prefixCls: string = 'et-tabs-default-bar';
  inkBarStyle: object = {};
  tabsBarStyle: object = {};
  ...
  @HostBinding('class.et-tabs-tab-bar-wrap')
  tabBarWrap = true;
  ...
  private setTabBarNavSwipingPosition(swipingDistance: number, swipingItemLength: number, viewportLength: number) {
    ...
    this.showPrev = this.tabBarNavSwipingPosition < 0;
  }
  ...
  private setTabBarNavSwipedPosition(swipingItemLength: number, viewportLength: number) {
    ...
    this.showPrev = this.tabBarNavSwipedPosition < 0;
    this.showNext = this.tabBarNavSwipedPosition + swipingItemLength * this.tabTitles.length - viewportLength > 0;
  }
